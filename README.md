...
## 🧪 Задача: Реализация простого логгера с фабрикой
### Везде где в проекте System.out.print заменить на logger.log сохранять в папку log в корне проекта
### Файлы логов (txt) не должны сохраняться в github !!!

## 🎯 Цель:
Научиться работать с записью в файл, использовать конструкторы и сделать фабрику объектов (без знания `Class<?>`).

---

## 📁 Что нужно реализовать:

### 1️⃣ SimpleLogger.java

Создать класс логгера, который будет записывать сообщения в файл с текущим временем и названием источника (например, `StudentService`).

#### 📦 Поля:
```java
private String logFile;        // Имя файла, в который пишем логи
private String sourceName;     // Имя класса, который использует логгер
```

#### ✅ Методы:
- **Конструктор** с двумя аргументами:
    - `logFile` – имя файла
    - `sourceName` – откуда пришло сообщение

- **Метод** `void log(String message)`
    - Записывает строку в файл:
      ```
      [время] [sourceName] сообщение
      ```
    - Пример:
      ```
      [2025-04-03T14:45:01.345] [StudentService] Студент создан
      ```

    - Используй `BufferedWriter` и `FileWriter` с режимом `append = true`
    - Не забудь обработку исключений (`try-catch`)

---

### 2️⃣ LoggerFactory.java

Создай **фабрику логгеров**, которая упрощает их создание.

#### ✅ Метод:
```java
public static SimpleLogger createLogger(String sourceName);
```

- Принимает `sourceName` — например, `"StudentService"`
- Формирует имя файла по шаблону:
  ```java
  String logFile = "log_" + sourceName + ".log";
  ```
- Возвращает новый `SimpleLogger`

---

### 3️⃣ Пример использования:

Создай любой класс, например:

📁 StudentService.java

```java
public class StudentService {
    SimpleLogger logger = LoggerFactory.createLogger("StudentService");
    
    public static void main(String[] args) {
        logger.log("Создан студент Иван Иванов");
        logger.log("Назначена оценка A");
    }
}
```
...
- 💡 Ты ещё не знаешь, что такое `Class<?>`, поэтому **в метод фабрики просто передаём имя `"StudentService"` как `String`**
- 📂 Логи будут сохраняться в файл с названием: `log_StudentService.log`
- Используй `LocalDateTime.now().toString()` для времени
- Для перехода на новую строку в файле — `writer.newLine()`
...
| Пункт                                   | Обязательно ✅      |
|----------------------------------------|---------------------|
| Логгер пишет сообщения в файл          | ✅                  |
| Формат сообщения: [время] [класс] ...  | ✅                  |
| Использован `BufferedWriter`           | ✅                  |
| Есть фабрика `LoggerFactory`           | ✅                  |
| Вызов логгера в отдельном классе       | ✅                  |
